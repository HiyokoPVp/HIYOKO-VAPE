...
run(function()
    ----------------------------------------------------------------
    -- Nuker モジュール（TargetBreakBed 対応版）
    ----------------------------------------------------------------
    ...
    local InstantBreak
    local LimitItem
    local TargetBreakBed

    -- 内部状態
    local customlist, parts = {}, {}
    local targetBreakPos = nil -- ユーザーが最初に手動で壊したブロックのワールド座標
    ...
    local function customHealthbar(self, blockRef, health, maxHealth, changeHealth, block)
        ...
        if not self.healthbarPart
            or not self.healthbarBlockRef
            or self.healthbarBlockRef.blockPosition ~= blockRef.blockPosition
        then
    ...
    -- ブロック破壊試行
    --   通常: 範囲内のブロックを順に処理
    --   TargetBreakBed ON: targetBreakPos に近い順にソートして処理
    ----------------------------------------------------------------
    local hit = 0
    ...
    local function attemptBreak(tab, localPosition)
        ...
        -- ▼ TargetBreakBed: ユーザーが壊した位置基準でソート
        if TargetBreakBed
            and TargetBreakBed.Enabled
            and targetBreakPos
        then
            local inRange = {}
            for _, v in tab do
            ...
            if #inRange > 1 then
                table.sort(inRange, function(a, b)
                    return (a.Position - targetBreakPos).Magnitude
                         < (b.Position - targetBreakPos).Magnitude
                end)
            end
        ...
        -- ▼ 破壊ループ
        for _, v in tab do
            local inRangeCheck = (v.Position - localPosition).Magnitude < Range.Value
            local breakable    = bedwars.BlockController:isBlockBreakable(
                { blockPosition = v.Position / 3 }, lplr
            )

            if inRangeCheck and breakable then
                -- 自分が置いたブロックはスキップ（SelfBreak OFF時）
                if not SelfBreak.Enabled
                    and v:GetAttribute('PlacedByUserId') == lplr.UserId
                then
                    continue
                end
                ...
                -- 道具制限
                if LimitItem.Enabled
                    and not (store.hand.tool and bedwars.ItemMeta[store.hand.tool.Name].breakBlock)
                then
                    continue
                end
                ...
                if path then
                    ...
                    for _, part in parts do
                        ...
                        if currentnode then
                            if currentnode == endpos then
                                part.BoxHandleAdornment.Color3 = Color3.new(1, 0.2, 0.2)
                            elseif currentnode == target then
                                part.BoxHandleAdornment.Color3 = Color3.new(0.2, 0.2, 1)
                            else
                                part.BoxHandleAdornment.Color3 = Color3.new(0.2, 1, 0.2)
                            end
                        end
    ...
    Breaker = vape.Categories.Minigames:CreateModule({
        ...
        Function = function(callback)
            if callback then
                ...
                -- ▼ TargetBreakBed: ユーザーのブロック破壊を監視
                --   最初に壊したブロックの位置のみ記録（以降は上書きしない）
                Breaker:Clean(vapeEvents.BreakBlockEvent.Event:Connect(function(data)
                    if data.player == lplr and targetBreakPos == nil then
                        targetBreakPos = data.blockRef.blockPosition * 3
                    end
                end))
    ...
    -- ▼ 新規オプション: TargetBreakBed
    TargetBreakBed = Breaker:CreateToggle({
        Name    = 'Target Break Bed',
        Tooltip = 'Starts nuking from the first block you manually broke',
    })
end)

...
-- TargetBreakBed: ユーザーが壊したブロックの位置を基準にソート
if TargetBreakBed.Enabled and targetBreakPos then
local sorted = {}
for _, v in tab do
...
return (a.Position - targetBreakPos).Magnitude < (b.Position - targetBreakPos).Magnitude
end)
end
tab = sorted
end
...
-- TargetBreakBed: ユーザーがブロックを壊したイベントを監視
if TargetBreakBed.Enabled then
...
Tooltip = 'Starts nuking from the last block you manually broke instead of the nearest low-HP block'
})
end)

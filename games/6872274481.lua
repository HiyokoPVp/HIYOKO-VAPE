...
run(function()
    local FallDamageBypass
    local TweenService = game:GetService("TweenService")
    
    FallDamageBypass = vape.Categories.Blatant:CreateModule({
        Name = 'FallDamageBypass',
        Function = function(callback)
            if callback then
                task.spawn(function()
                    while FallDamageBypass.Enabled and entitylib.isAlive do
                        -- 1. DamageBoostがオフならオンにする
                        local db = vape.Modules['DamageBoost']
                        if db and not db.Enabled then
                            pcall(function() db:Toggle() end)
                        end
                        
                        local root = entitylib.character.RootPart
                        local startY = root.Position.Y
                        local targetY = startY + 40
                        
                        -- 2. 1秒で上に40 studs移動
                        local tweenUp = TweenService:Create(root, TweenInfo.new(1, Enum.EasingStyle.Linear), {
                            CFrame = CFrame.new(root.Position.X, targetY, root.Position.Z)
                        })
                        tweenUp:Play()
                        tweenUp.Completed:Wait()
                        
                        -- 3. 1秒で下に40 studs移動（元の位置に戻る）
                        local tweenDown = TweenService:Create(root, TweenInfo.new(1, Enum.EasingStyle.Linear), {
                            CFrame = CFrame.new(root.Position.X, startY, root.Position.Z)
                        })
                        tweenDown:Play()
                        
                        -- 4. 着地前後の2秒間、落下ダメージを無効化（Landedステートを強制）
                        local noFallStart = tick()
                        while tick() - noFallStart < 2 and FallDamageBypass.Enabled do
                            if entitylib.isAlive then
                                pcall(function()
                                    entitylib.character.Humanoid:ChangeState(Enum.HumanoidStateType.Landed)
                                    -- 速度をリセットして物理演算によるダメージを防ぐ
        ...
        Tooltip = 'Teleports up and down to bypass fall damage and keep DamageBoost active.'
    })
end)
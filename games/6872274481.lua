...
run(function()
	...
	-- リソース集計ヘルパー (ShowThemResourceのロジックを流用)
	local function getPlayerResources(plr)
		if not plr then return {Iron=0, Gold=0, Diamond=0, Emerald=0} end
		local counts = {Iron=0, Gold=0, Diamond=0, Emerald=0}
		
		local invFolder = replicatedStorage:FindFirstChild("Inventories")
		if invFolder then
			local plrInv = invFolder:FindFirstChild(plr.Name)
			if plrInv then
				for _, item in plrInv:GetChildren() do
					local name = item.Name:lower()
					local amount = item:GetAttribute("Amount") or 1
					if name:find("iron") then counts.Iron += amount
					elseif name:find("gold") then counts.Gold += amount
					elseif name:find("diamond") then counts.Diamond += amount
					elseif name:find("emerald") then counts.Emerald += amount end
				end
			end
		end
		...
		for _, chest in collectionService:GetTagged("chest") do
			if chest:GetAttribute("PlacedByUserId") == plr.UserId then
				local val = chest:FindFirstChild("ChestFolderValue")
				local folder = val and val.Value
				if folder then
	...
	local function formatResources(res)
		return string.format("[Fe:%d Au:%d Di:%d Em:%d]", res.Iron, res.Gold, res.Diamond, res.Emerald)
	end

	local function addLog(text, color)
		table.insert(LogList, 1, {Text = text, Color = color})
		if #LogList > MaxLogs then table.remove(LogList, MaxLogs + 1) end
		refreshUI()
	end

	-- UI構築
	local ScreenGui = Instance.new("ScreenGui")
	ScreenGui.Name = "BattleConsole"
	...
	MainFrame.BackgroundColor3 = Color3.fromRGB(20, 20, 20)
	MainFrame.BorderSizePixel = 0
	MainFrame.Position = UDim2.fromOffset(Config.PositionX, Config.PositionY)
	MainFrame.Size = UDim2.fromOffset(Config.SizeW, Config.SizeH)
	...
	TitleLabel.Text = "Battle Console (Draggable)"
	...
	UserInputService.InputChanged:Connect(function(input)
		if dragging and input.UserInputType == Enum.UserInputType.MouseMovement then
			...
			-- スライダーとの同期 (オプション値を更新)
			Config.PositionX = MainFrame.AbsolutePosition.X
			Config.PositionY = MainFrame.AbsolutePosition.Y
		elseif resizing and input.UserInputType == Enum.UserInputType.MouseMovement then
			...
			Config.SizeW = MainFrame.AbsoluteSize.X
			Config.SizeH = MainFrame.AbsoluteSize.Y
		end
	end)

	-- モジュール定義
	ConsoleLog = vape.Categories.Utility:CreateModule({
		Name = "Battle Console",
		Function = function(callback)
			if callback then
				ScreenGui.Parent = lplr.PlayerGui
				addLog("Console Started...", Color3.fromRGB(100, 100, 100))
			else
				ScreenGui.Parent = nil
			end
		end,
		Tooltip = "Shows kills, bed breaks, and purchases with resources."
	})

	-- ★ オプション: 位置とサイズ (Vector2の代わりにX/Y/W/Hスライダーで実装)
	ConsoleLog:CreateSlider({
		Name = 'Position X', Min = 0, Max = 1920, Default = Config.PositionX, Decimal = 0,
		Function = function(val) Config.PositionX = val; MainFrame.Position = UDim2.fromOffset(Config.PositionX, Config.PositionY) end
	})
	ConsoleLog:CreateSlider({
		Name = 'Position Y', Min = 0, Max = 1080, Default = Config.PositionY, Decimal = 0,
		Function = function(val) Config.PositionY = val; MainFrame.Position = UDim2.fromOffset(Config.PositionX, Config.PositionY) end
	})
	ConsoleLog:CreateSlider({
		Name = 'Size Width', Min = 200, Max = 800, Default = Config.SizeW, Decimal = 0,
		Function = function(val) Config.SizeW = val; MainFrame.Size = UDim2.fromOffset(Config.SizeW, Config.SizeH) end
	})
	ConsoleLog:CreateSlider({
		Name = 'Size Height', Min = 150, Max = 800, Default = Config.SizeH, Decimal = 0,
		Function = function(val) Config.SizeH = val; MainFrame.Size = UDim2.fromOffset(Config.SizeW, Config.SizeH) end
	})
	ConsoleLog:CreateSlider({
		Name = 'Opacity', Min = 0, Max = 1, Default = Config.BgTransparency, Decimal = 10,
		Function = function(val) Config.BgTransparency = val; MainFrame.BackgroundTransparency = val end
	})

	-- イベント接続
	vape:Clean(vapeEvents.EntityDeathEvent.Event:Connect(function(deathTable)
		if not ConsoleLog.Enabled then return end
		local killer = playersService:GetPlayerFromCharacter(deathTable.fromEntity)
		local victim = playersService:GetPlayerFromCharacter(deathTable.entityInstance)
		if killer and victim then
			local res = getPlayerResources(victim)
			addLog(string.format("%s killed %s %s", killer.Name, victim.Name, formatResources(res)), Color3.new(1,1,1))
		end
	end))
	...
	vape:Clean(vapeEvents.BedwarsBedBreak.Event:Connect(function(bedTable)
		...
		local teamId = bedTable.brokenBedTeam.id
		local queueType = store.queueType or 'bedwars_test'
		local teamMeta = bedwars.QueueMeta[queueType]
		local teamName = (teamMeta and teamMeta.teams and teamMeta.teams[tonumber(teamId)]) and teamMeta.teams[tonumber(teamId)].displayName or "Unknown"
		addLog(string.format("BED BROKEN: %s's bed by %s", teamName, breaker and breaker.Name or "?"), Color3.fromRGB(255, 80, 80))
	end))

	-- 購入ログ (Notificationフック)
	local oldSendInfo = bedwars.NotificationController.sendInfoNotification
	bedwars.NotificationController.sendInfoNotification = function(self, data)
		if ConsoleLog.Enabled and data.message and data.message:find("Bought") then
			addLog(data.message, Color3.fromRGB(100, 255, 100))
		end
		return oldSendInfo(self, data)
	end
	ConsoleLog:Clean(function() bedwars.NotificationController.sendInfoNotification = oldSendInfo end)
end)